...
record = pd.DataFrame(columns=["Time", "DataDifferencesSum", "MultDataDifferencesSum", "MultDataDifferencesHistoricalSum", "Odds"])

# Set "All-Time" in the first row for the "Time" column
record.loc[0, "Time"] = "All-Time"

# Set "W: 0, L: 0, %: 0" in the rest of the rows for all other columns
record.iloc[:, 1:] = "W: 0, L: 0, %: 0"
...
#record.to_sql(name="record", con=engine, if_exists='fail', index=False)
