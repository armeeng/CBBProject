...
def add_noise_to_table(table):
    noise = pd.DataFrame(np.random.normal(0, 0.1, size=table.shape), columns=table.columns)
    return table + noise

def multiply_noise_to_table(table):
    noise = pd.DataFrame(np.random.normal(0, 1, size=table.shape), columns=table.columns)
    return table * noise
